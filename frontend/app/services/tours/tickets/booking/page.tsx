...
export default function TicketBookingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    await handleFormSubmit(
      ...
      setError,
      () => {
        setForm({ name: "", contact: "", travelDate: "", returnDate: "", passengers: 1, ticketType: "", departure: "", destination: "", requests: "" });
  ...
  return (
    <PageTransition>
      <div className="max-w-2xl mx-auto py-12 px-4 animate-fade-in">
        <h1 className="text-3xl font-bold mb-6 flex items-center gap-2 text-blue-800"><Ticket className="w-7 h-7 text-blue-500" /> Ticket Booking</h1>
        {success && (
          <div className="mb-4 p-4 bg-blue-100 text-blue-800 rounded-lg border border-blue-200">
            ...
            <p className="mt-1">Ticket booking submitted successfully! We've opened WhatsApp and your email client. We'll contact you soon.</p>
          </div>
        )}
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" name="travelDate" value={form.travelDate} onChange={handleChange} required className="w-full border rounded p-2" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Return Date (Optional)</label>
              <input type="date" name="returnDate" value={form.returnDate} onChange={handleChange} className="w-full border rounded p-2" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
            ...
            <div>
              ...
              <select name="ticketType" value={form.ticketType} onChange={handleChange} required className="w-full border rounded p-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="departure" value={form.departure} onChange={handleChange} required className="w-full border rounded p-2" placeholder="Enter departure location*" />
            </div>
            <div>
